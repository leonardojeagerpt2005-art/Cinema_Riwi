...
// Formas de las respuestas de la API countriesnow.space
interface IsoCountryJson {
  name: string;
  iso2: string;
}

interface ApiStateJson {
  name: string;
  state_code: string;
}
...
export async function fetchCountries(): Promise<Country[]> {
  ...
  const json = (await response.json()) as { data: IsoCountryJson[] };

  return json.data
    .filter((pais) => SUPPORTED_COUNTRIES.includes(pais.name))
    .map((pais) => ({
      name: pais.name,
      iso2: pais.iso2,
    }));
...
export async function fetchCities(
  ...
  return json.data.map((nombreCiudad) => ({
    name: nombreCiudad,
